...
- `reports/` — 각 실행일자별 입찰 스냅샷 보고서 (사람이 읽는 마크다운)
- `trends_ledger.json` — **별도 루틴**: 전세계 BHS/PBB(+PC-Air·PWS·AC-GPS) **사고 사례 및 신기술 동향** 기계판독용 원장(입찰과 무관, 뉴스/제작사 자료 추적)
- `trend_reports/` — 사고·신기술 동향 스냅샷 보고서 (사람이 읽는 마크다운). 평일은 신규분만, 금요일은 주간 종합.
...
## 실행 이력 (사고·신기술 동향 트래커 — `trends_ledger.json` / `trend_reports/`)
- **2026-08-12 (수)** — baseline 최초 실행. BHS 6건 사고 + 6건 신기술, PBB 5건 사고 + 9건 신기술, PBB 부대설비 5건 사고 + 7건 신기술 기록. 일부 제작사 도메인은 egress 차단으로 2차 보도로 교차확인.
